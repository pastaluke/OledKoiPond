...
| E13-4 | Spline "muscle sim" — sliding **waist pivot**; back half flexes/wags from it (propulsion fake), inherits front-half curve under forward momentum; universal (fish/eel/air-swimmer). Rides: `{t,w,pivot}` schema bump + `schemaVersion` + **trim `upgradeCreature`** (drop legacy branch, keep thin import-seam boundary). Tags: Glide-depth→1 (drag-only brake), reconsider Arc(sm)/(lg), turn-away-not-brake collisions. **Design/research — see plan doc** | ⬜ Design |
