...
5. **Forced symmetry** — profile is mirrored about the spine; no flat belly /
   rounded back, no asymmetry. → Phase 2 schema (optional separate top/bottom).
...
CreatureDef = {
  ...
  spline: {
    ...
    // Optional: wTop/wBottom to break symmetry (defaults to w for both).
    points: [
    ...
    symmetry: 'mirror' | 'split',  // 'mirror' = wTop=wBottom; 'split' = independent
  },
  ...
  patterns: {
    variations: [
      {
        name: 'kohaku',
        weight: 0.5,           // spawn probability share
...
- **Dynamic count, min 3.** Drop `PT_LABELS`; points are positional (`Point N,
  t=…`).
- **Movable endpoints.** First/last t become editable. Recommended interpretation
  (see Open Questions): the profile's t-range is **renormalized** so the body
  always spans tail-tip→snout regardless of endpoint t — i.e. "move this point
  left/right" stays inside [0,1] and never produces negative-width extrapolation.
...
- Pattern **variations** per class + **spawn-weight** UI; individuals roll a
  variation at spawn.

*Done when:* a class can define ≥2 variations, fish spawn with the right mix, and
blobs recolor automatically from palette variety.
...
- **Endpoint-t semantics** (Phase 1): recommended = renormalize the profile
  t-range to the actual point span so the body always spans end-to-end. Alt:
  clamp endpoints to `[0, p1)` / `(pN-1, 1]` and guard width extrapolation.
...
- **Symmetry break** (`split` top/bottom): schema reserves it; defer the editor
  UI until there's demand.
